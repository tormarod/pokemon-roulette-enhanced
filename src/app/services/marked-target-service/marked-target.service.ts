 ...
 * that team member is forced to lead the next real battle's prep screen (every
 * other pick is disabled), then clears once that battle resolves. Persisted so
 * a reload can't shake off the mark. Tracked by team index at mark time (like
 * leadIndex), not object identity — acceptable because nothing reorders the
 * team between a threat draw and the following battle's prep screen.
 */
