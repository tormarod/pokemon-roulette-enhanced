...
- **Pre-spin X Attack**: optionally spend an X Attack *before* the wheel spins, rather than only reactively after a loss. X Attack becomes a one-time, consumed power boost (instead of Classic mode's quirk of every X Attack in your bag passively applying every battle, forever, without being used up). Potions aren't part of the prep — they're already used automatically (weakest first) as a retry the moment a spin lands No, so there's nothing to gain by committing one up front.
...
- **The Market.** An always-available shop (a button in the team strip, like the PC) that you can open any time **outside** combat — it never advances the round. Spend Coins on potions (Potion/Super/Hyper), an X Attack, a Revive, a Rare Candy, a **Bicycle** (a pricier, guaranteed extra threat-free adventure step), or a random **Ability Capsule** (bagged like a found capsule, then assigned from the PC). Rows you can't afford are disabled. It's deliberately closed during a battle spin or the prep panel, so a purchase can never react to a shown loss or to odds you've already seen — the same anti-reroll fairness rule the rest of the run persistence follows.

All the coin amounts and Market prices are tunable dials in [`economy-config.ts`](src/app/main-game/roulette-container/economy-config.ts). Classic mode has no coins or Market — it's unchanged.
