...
With New Experience Mode on, the adventure step between fights also changes from a passive wheel spin, paced by a **Danger meter** that stays visible throughout the whole roster-building stretch between fights — not just on the step where it's rolled — so its trend is always in view; it's hidden during actual battles (which have their own odds) and before the adventure starts. Hover or tap it for a tooltip explaining the mechanic. The meter climbs with rounds survived, drops sharply whenever a threat is drawn, and recovers gradually on safe steps; three threats in a row force the next step safe (a hard pity), so danger never spirals unrecoverably. Any reward that hands you extra adventure steps — the **multitask** reward (two extra steps) or the **Running Shoes** item (one extra step) — makes those bonus steps guaranteed threat-free (the meter still climbs across them), so a reward can never be the thing that ambushes you. On a **reward** step, 3 candidate outcomes are drawn and you pick one — the familiar catch, trade, legendary encounter, Team Rocket, etc. On a **threat** step, there's no picker: a single outcome is drawn at random from a pool of seven and its effect just happens to you, with no way to steer toward the least-bad option — a Team Rocket ambush (the same steal/run/defeat mini-wheel, framed as a threat instead of a gamble), an item theft (loses one random item, or finds nothing with an empty bag), a bad omen (extra risk carried into your very next battle only, then it clears), a Forced Retreat (benches a weak-biased team Pokémon to PC storage, locked there — undraggable — until your next real battle resolves), a Spooked scare (spikes the Danger meter back up, no item/Pokémon cost), a Marked Target (one random team Pokémon is forced to lead your next battle's prep screen, every other pick disabled), and a Poké Ball Malfunction (your very next catch attempt has a chance to fail and the Pokémon gets away). Classic mode's adventure step is unaffected — still a single wheel spin.
