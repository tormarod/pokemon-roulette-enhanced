...
- New Experience Mode also adds a curated set of 18 Pokémon **abilities** (one per type) that shift a battle's odds, plus real **stakes on a rival loss**: your lead faints and needs a Revive to fight again, instead of the loss being free — see [Abilities & the faint mechanic](#abilities--the-faint-mechanic) below.
...
Also New Experience Mode-only: a small, curated roster of 18 Pokémon **abilities** — one hand-picked species per type (Charizard's Blaze, Gengar's Levitate, Mightyena's Intimidate, and so on) — each nudging the Yes/No odds if that species is on your team, shown as a badge next to the matchup strip so an odds swing is never unexplained. Abilities apply to every battle type (gym, rival, Elite Four, Champion); Classic mode never has an active ability, regardless of team composition.

Rival battles specifically also gained real stakes: a rival loss used to be free (the run just continued). Now, on a loss with no retries or potions left, your chosen **lead Pokémon faints** — it's immediately pulled from your team (freeing a slot) and moved to the PC, shown greyed-out with a "Fainted" badge and undraggable back into the team until revived with a **Revive** item (a new, rarer-than-Hyper-Potion find). If fainting would leave your team empty, the run ends there, same as any other loss. A lead with the Sturdy ability survives a would-be faint instead, once per battle. Gym, Elite Four, and Champion losses are unaffected — a loss there still ends the run outright, exactly as before.
