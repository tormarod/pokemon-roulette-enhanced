...
- **Approved plans live in `docs/plans/*.md` (gitignored, local only).** When a nontrivial feature is planned, write the approved plan to a file there (self-contained, with a "current system" section so an execution session needn't re-research). Execution sessions implement against that file and update its status/checkboxes as phases complete. Ask to continue after each phase is done, do not run multiple phases in one go. This is how work is handed between sessions/models — not via one long thread, and not something that needs to ship in the repo.
