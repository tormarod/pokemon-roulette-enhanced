...
**Battle math is centralized and shared.** All four battle types (gym, rival, Elite Four, Champion) compute their Yes/No wheel odds through `TypeMatchupService.calcTeamMatchupTotals()` (`src/app/services/type-matchup-service/`), invoked via `BaseBattleRouletteComponent.buildVictoryOdds()` which each battle roulette component extends. The per-Pokémon type-matchup delta (`ceil(power / 4)` per net-score point — see `TypeMatchupService.getMemberDelta()`; the older `ceil(power / 2)` "strong" magnitude is the special case of a net score of 2) depends only on that Pokémon, never team composition — preserve that invariant if touching this logic (see README "Battle balancing" for why).

**Pokémon data is fully local; only the sprite images are remote.** `nationalDexPokemon` (`src/app/services/pokemon-service/national-dex-pokemon.ts`) is the local source of truth for id/name/type/power for all Pokémon, and each entry carries its official-artwork URLs as static data. Alternate forms (ids > 10000 — Alolan/Galarian/mega/... variants in `pokemon-forms.ts` and `pokemon-mega-forms.ts`) are built at runtime with `sprite: null` and get their URLs from `officialArtworkSprites()` (`pokemon-service/official-artwork.ts`), which derives them from the id **synchronously**. Keep it that way: popups like the evolution/trade ones read `sprite.front_default` on the line right after the form is built, so an async lookup there renders an empty tile. There is no runtime call to `pokeapi.co` — the sprite images themselves are just `<img>` URLs on `raw.githubusercontent.com`.
...
- **UI changes must look right in all three themes.** `ThemeService` (`src/app/services/theme-service/`) applies one of `theme-starters` (dark, tiled background), `theme-plain-dark`, or `theme-plain-light` as a class on `<body>`; `isDark$` emits true for the two dark themes. Any new or changed UI element needs readable contrast in every theme — never hardcode a text color without also setting (or theme-scoping) its background, and vice versa; dark text on a dark theme (or light-on-light) is the classic failure. Theme-scope styles with `body.theme-* …` selectors — but those must live in the global `src/styles.css`, not component CSS: Angular's emulated view encapsulation makes component-scoped `body.theme-x .foo` rules silently never match (see the comments in `styles.css` for examples). Same trap applies to third-party content rendered outside your component's template (ngb modals/toasts). Before calling a UI change done, verify it in a dark theme *and* `plain-light`.
...
- **Version bumps follow app-semver — don't bump MINOR for everything.** The version is really an ordered cursor for the What's New modal, but keep it disciplined so numbers don't gallop (the 3.0→3.14 run bumped MINOR for balance tweaks and bug fixes, which is what to avoid):
  - **MAJOR** — a mode-defining overhaul (e.g. New Experience Mode was 3.0.0; removing Classic mode was 4.0.0). Rare.
...
- **Write plans concrete enough for a mid-tier model (Sonnet, used for execution here) to implement with no extra reasoning.** Exact file paths, function/method names, signatures, and the specific edit — spell out the algorithm, don't say "aggregate appropriately". **Ask the user about open design questions — do not decide them yourself.** When a plan has real design choices, surface them to the user (with a recommendation and the trade-offs) and let them choose, then bake the chosen answers into the plan so it ends up fully decided. Don't unilaterally pick and don't leave "recommend X, decide later" for the implementer — the plan must be concrete, but the decisions are the user's to make. Give ordered, checkable steps and explicit acceptance tests as input→expected-output. Keep the rationale trimmed to just enough "why" to avoid a wrong turn — cut the design essays and worked-example digressions.
- **Pending changes, bug fixes, and TODOs go in `docs/todo/backlog.md` (tracked in git).** Add an entry whenever something noteworthy comes up mid-session that isn't worth fixing immediately or needs a decision first. Promote an entry to a full `docs/plans/*.md` file if it grows into a real multi-phase plan.
- **Keep the backlog current — required.** Whenever a change ships (or you notice a listed item is already resolved in the code), **delete the corresponding backlog entry as part of that same work**. Never leave done items in `backlog.md`. Before adding new entries or when asked to review the backlog, verify each open item against the current code and remove any that are already done.
